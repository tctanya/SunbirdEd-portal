    ...
    private fetchContents() {
        ...
        this.searchService.contentSearch(option)
            ...
            .subscribe(data => {
                ...
                // this.newContentList = this.contentList.map((data1) => ({ contentType: data1.contentType, content: [data1] }));
                // // this.filteredContent = this.newContentList.filter((data) => data.contentType)
                ...
                // // const counts = _.countBy(this.newContentList, 'contentType')
                // // console.log("filtereddata************", _.filter(this.newContentList, x => counts[x.contentType] > 1));
                ...
                // let result = [];
                // this.newContentList.forEach((elem, i) => {
                //     let match = result.find(r => r.contentType === elem.contentType);
                //     if (match) {
                //         console.log("IData", result[i - 1]);
                //         if (result[i - 1] && result[i - 1].content) {
                //             result[i - 1].content.push(...elem.content)
                //         }
